...
public class CalculatorResidual {
  ...
  protected static SortedMap<Double, Map<Float, Integer>> getMapResidual(
      final List<Float> dosagesMedicamentList,
    ...
    for (Float dosage : dosagesMedicamentList) {
      if (requiredDose >= dosage) {
        ...
        if (mapDoseQuantityPackage.containsKey(dosage)) {
          mapDoseQuantityPackage.replace(dosage,
              (mapDoseQuantityPackage.get(dosage) + quantityPackage));
        } else {
      ...
      if (tempMapDoseQuantityPackage.containsKey(dosage)) {
        tempMapDoseQuantityPackage.replace(dosage,
            (tempMapDoseQuantityPackage.get(dosage) + quantityPackage));
      } else {
